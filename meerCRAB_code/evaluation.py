...
from meerCRAB_code.plot import plot_confusion_matrix, plot_roc, optimsation_curve, feature_maps, plot_images
import pandas as pd
from meerCRAB_code.util import ensure_dir
__version__='2.0.0'


def model_prediction(fit_model, odir, model_name, X_test, y_test, model_path='./meerCRAB_model/',classes=["Bogus" , "Real"], cm_norm=False,load_model=False):
    '''
    ...
    INPUTS:
        ...
        model_name: if load_model is True, model_name = 'NET1', 'NET2','NET3', Either of them
    ...
    plot_confusion_matrix(conf_mat, classes_types=classes, ofname=os.path.join(odir, "confusion_matrix.pdf"), normalize=cm_norm)    
    plot_roc(fpr, tpr, auc, ofname=os.path.join(odir, "ROC.pdf"))
